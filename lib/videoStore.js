...
const LOCAL_DIR = path.join(process.cwd(), ".data", "videos");
...
async function getBlobStore() {
  ...
  return getStore("ferraio-videos");
}
...
export async function saveVideo(arrayBuffer, contentType) {
...
export async function getVideo(key) {
  if (useNetlifyBlobs()) {
    ...
    return {
      ...
      contentType: result.metadata?.contentType || "video/mp4",
    };
  }
